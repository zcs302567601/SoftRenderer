...
public class ZbufferFrame extends BaseFrame implements MouseMotionListener {
	/**
	 * 
	 */
	...
	public void init() {
		...
		box2 = new Box(new Vector3(0.2f, 0.4f, 0.5f));
	...
	void AddRenderList(Matrix4x4 v, Box box)
	{
		...
		for(int i = 0; i < triangles.length; i++)
		{
			...
			TriangleFace face = new TriangleFace(new Vertex3[]{vertexes[index1], vertexes[index2], vertexes[index3]});
			renderList.add(face);
		}
	}
	...
	void RenderRenderList(SoftRaster raster)
	{
		for(int i = 0;i < renderList.size(); i++)
		{
			...
			Vertex v1 = new Vertex(face.vertices[0].screenPos, face.vertices[0].perspectivePosition, face.vertices[0].color);
			Vertex v2 = new Vertex(face.vertices[1].screenPos, face.vertices[1].perspectivePosition, face.vertices[1].color);
			Vertex v3 = new Vertex(face.vertices[2].screenPos, face.vertices[2].perspectivePosition, face.vertices[2].color);
