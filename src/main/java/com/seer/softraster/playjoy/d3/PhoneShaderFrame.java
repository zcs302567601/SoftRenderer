...
import com.seer.shader.PhoneShader;
...
public class PhoneShaderFrame extends BaseFrame implements MouseMotionListener{
	
	private static final long serialVersionUID = 1L;

	public static void main(String[] args) {
		new PhoneShaderFrame().start();
	}
...
//			this.addMouseMotionListener(this);
		...
		plane = new Plane(new Vector3(0, 0, 0), new PhoneShader(camera));
		try {
	...
	void addRenderList(Matrix4x4 v, Plane plane)
	{
		...
		for(int i = 0; i < triangles.length; i++)
		{
			...
			TriangleFace face = new TriangleFace(new Vertex3[]{vertexes[index1], vertexes[index2], vertexes[index3]}, triangle.uv, plane.shader);
			renderList.add(face);
		}
	}
