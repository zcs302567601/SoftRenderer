...
import com.seer.shader.FragmentPhoneShader;
...
public class FragmentPhoneFrame extends BaseFrame implements MouseMotionListener{
	
	private static final long serialVersionUID = 1L;

	public static void main(String[] args) {
		new FragmentPhoneFrame().start();
	}
...
//			this.addMouseMotionListener(this);
		...
		plane = new Plane(new Vector3(0, 0, 0), new FragmentPhoneShader(camera));
		try {
	...
	void addRenderList(Matrix4x4 v, Plane plane)
	{
		...
		for(int i = 0; i < triangles.length; i++)
		{
			...
			TriangleFace face = new TriangleFace(new Vertex3[]{vertexes[index1], vertexes[index2], vertexes[index3]}, triangle.uv, triangle.normal, plane.shader);
			renderList.add(face);
		}
	}
	...
	void renderRenderList(SoftRaster raster)
	{
		for(int i = 0;i < renderList.size(); i++)
		{
			...
			Vertex v1 = new Vertex(face.vertices[0].screenPos, face.vertices[0].worldPosition, face.uv[0], face.vertices[0].perspectivePosition, face.vertices[0].normal, face.vertices[0].color);
			Vertex v2 = new Vertex(face.vertices[1].screenPos, face.vertices[1].worldPosition, face.uv[1], face.vertices[1].perspectivePosition, face.vertices[1].normal, face.vertices[1].color);
			Vertex v3 = new Vertex(face.vertices[2].screenPos, face.vertices[2].worldPosition, face.uv[2], face.vertices[2].perspectivePosition, face.vertices[2].normal, face.vertices[2].color);
			raster.fillPolygon(new Vertex[]{v1, v2, v3}, face.shader);
